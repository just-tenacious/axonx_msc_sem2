            ...
            <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
                ...
                <div className="pro-card p-0 overflow-hidden shadow-2xl">
                    <div className="p-6 border-b border-[var(--border-color-light)] flex flex-wrap items-center gap-4">
                        <div className="relative flex-1 min-w-[200px]"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--text-muted)]" size={16} /><input type="text" placeholder="Search entries…" className="pro-input w-full pl-11 h-12" value={searchTerm} onChange={e => setSearchTerm(e.target.value)}/></div>
                        <div className="flex bg-[var(--bg-color)] p-1 rounded-2xl border border-[var(--border-color)]">
                            {['All', 'Pending', 'Confirmed', 'Completed'].map(s => (
                                <button key={s} onClick={() => setSelectedStatus(s)} className={`px-4 py-2 text-[0.6rem] font-black uppercase rounded-xl transition-all ${selectedStatus === s ? 'bg-white text-indigo-500 shadow-sm' : 'text-[var(--text-muted)]'}`}>{s}</button>
                            ))}
                        </div>
                    </div>
        ...
        return (
            <div className="animate-in zoom-in-95 duration-500 max-w-5xl mx-auto space-y-8">
                <div className="flex items-center gap-4">
                    ...
                    <nav className="flex items-center gap-3 text-[0.7rem] font-black uppercase tracking-widest text-[var(--text-muted)]"><button onClick={() => setView('dashboard')}>Analytics</button><ChevronRight size={12} className="opacity-30" /><button onClick={() => setView('list')}>Records</button><ChevronRight size={12} className="opacity-30" /><span className="text-blue-500 italic">Dossier</span></nav>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-1 space-y-6">
                        <div className="pro-card p-12 bg-white shadow-xl rounded-[48px] border-0 relative overflow-hidden text-center">
                            <div className={`absolute top-0 inset-x-0 h-2 bg-gradient-to-r ${getStatusStyle(a.status).split(' ')[0]}`}></div>
                            <h4 className="text-[0.6rem] font-black text-slate-400 uppercase tracking-widest mb-10">Current Status</h4>
                            <div className={`p-8 rounded-[36px] border-2 shadow-inner inline-block w-full ${getStatusStyle(a.status)}`}><p className="text-3xl font-black italic tracking-widest">{a.status}</p></div>
                            <div className="mt-12 p-8 bg-slate-50 rounded-[32px] border border-slate-200 text-center space-y-4">
                                <div className="p-3 bg-white rounded-xl text-indigo-500 w-fit mx-auto shadow-sm"><ShieldCheck size={24} /></div>
                                <div><h4 className="text-[0.65rem] font-black text-slate-800 uppercase mb-2">Node Verification</h4><p className="text-[0.65rem] font-bold text-slate-600 italic">Verified for this clinical node.</p></div>
                            </div>
                        </div>
                        <button onClick={() => openEdit(a)} className="w-full py-5 bg-indigo-600 text-white font-black rounded-[32px] shadow-2xl hover:scale-[1.02] flex items-center justify-center gap-3 transition-all uppercase text-xs tracking-widest">Update</button>
                    </div>
                    <div className="lg:col-span-2 space-y-6">
                        <div className="pro-card p-12 bg-white shadow-xl border-0 rounded-[48px]">
                            <h4 className="text-[0.65rem] font-black text-[var(--text-muted)] uppercase tracking-widest mb-12 flex items-center gap-3"><User size={18} className="text-indigo-500" /> Patient Identity</h4>
                            <div className="flex items-start gap-8">
                                <div className="w-28 h-28 rounded-[40px] bg-slate-50 border-4 border-white shadow-2xl overflow-hidden"><img src={a.patientId?.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${a.patientId?.username}`} className="w-full h-full object-cover" /></div>
                                <div className="flex-1 space-y-8 text-left">
                                    <div><p className="text-[0.6rem] font-black text-slate-400 uppercase mb-1.5 ml-1">Full Legal Name</p><p className="text-2xl font-black text-slate-800 tracking-tight">{a.patientId?.name}</p><p className="text-xs font-bold text-blue-500 mt-1 italic">{a.patientId?.email}</p></div>
                                    <div className="p-6 bg-slate-50 rounded-[32px] border flex items-center gap-4 text-xs font-bold text-slate-500 italic"><Info size={20} className="text-indigo-500 flex-shrink-0" /> Verified {a.patientId?.role} node.</div>
                                </div>
                            </div>
                        </div>
                        <div className="pro-card p-12 bg-white shadow-xl border-0 rounded-[48px] border-l-[16px] border-indigo-500">
                            <h4 className="text-[0.65rem] font-black text-indigo-500 uppercase tracking-widest mb-12 flex items-center gap-3"><Stethoscope size={18} /> Medical Lead</h4>
                            <div className="flex items-start gap-8">
                                ...
                                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-10 text-left">
                                    <div><p className="text-[0.6rem] font-black text-slate-400 uppercase mb-1.5 ml-1">Assigned Doctor</p><p className="text-xl font-black text-indigo-600">Dr. {a.doctorId?.name}</p></div>
                                    <div><p className="text-[0.6rem] font-black text-slate-400 uppercase mb-1.5 ml-1">Endpoint Identification</p><p className="text-sm font-black text-slate-500 font-mono italic">{a.doctorId?.email}</p></div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div className="animate-in slide-in-from-bottom-6 duration-500 max-w-4xl mx-auto space-y-8">
                ...
                <div className="pro-card p-14 bg-white shadow-2xl border-0 rounded-[56px] relative overflow-hidden">
                    ...
                    <form onSubmit={handleUpdate} className="grid grid-cols-2 gap-12 text-left">
                        ...
                        <div className="space-y-3"><label className="text-[0.65rem] font-black text-slate-400 uppercase tracking-widest ml-1">Slot Time</label><input type="text" required className="pro-input w-full h-16 px-6 rounded-[24px] font-bold" value={editForm.time} onChange={e => setEditForm(f => ({...f, time: e.target.value}))}/></div>
